...
public class ForkedSimplePlanner implements IPlanner {
		...
		for (Explanation next : explanations) {
			if (next instanceof Explanation.MissingIU) {
				...
				if (missingIU.req instanceof IRequiredCapability && "A.PDE.Target.Platform".equals(((IRequiredCapability) missingIU.req).getNamespace())) //$NON-NLS-1$
					forTargets.add(new Status(IStatus.ERROR, DirectorActivator.PI_DIRECTOR, Explaination_getUserReadableName(missingIU.iu)));
			}
		}
	...
	public ForkedSimplePlanner(IProvisioningAgent agent) {
	...
	private Object getSolutionFor(ProfileChangeRequest profileChangeRequest, ProvisioningContext context, IProgressMonitor monitor) {
		...
		try {
			...
			Projector projector = new Projector(slice, newSelectionContext, getNonGreedyIUs(slicer), satisfyMetaRequirements(profileChangeRequest.getProfileProperties()));
	...
	private static Set<IInstallableUnit> getNonGreedyIUs(Slicer slicer) {
		try {
			if (Slicer_nonGreedyIUs_Field == null) {
				Slicer_nonGreedyIUs_Field = Slicer.class.getDeclaredField("nonGreedyIUs");
				Slicer_nonGreedyIUs_Field.setAccessible(true);
			}
			return (Set<IInstallableUnit>)Slicer_nonGreedyIUs_Field.get(slicer);
		} catch (Throwable t) {
			t.printStackTrace();
			return null;
		}
	}
	//some copy/paste from Explaination
	protected static String Explaination_getUserReadableName(IInstallableUnit iu) {
		if (iu == null)
			return ""; //$NON-NLS-1$
		String result = Explaination_getLocalized(iu);
		if (result == null)
			return iu.toString();
		return result + ' ' + iu.getVersion() + " (" + iu.toString() + ')'; //$NON-NLS-1$
	}
	private static String Explaination_getLocalized(IInstallableUnit iu) {
